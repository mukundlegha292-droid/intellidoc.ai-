...
function AIWorkspacePage() {
  ...
  const handleFile = async (file?: File) => {
    if (!file) return; setProcessing(true); setError(""); setStatus(`Preparing ${extOf(file.name).toUpperCase() || "FILE"}…`);
    try { processResult(await extractFile(file, setStatus), file.name); } catch (e) { console.error(e); setStatus("Import failed"); setError("This file could not be processed in the browser."); } finally { setProcessing(false); }
  };
  ...
  const ask = async (text = question) => {
    ...
    setQuestion(""); setProcessing(true); setError(""); setStatus("Thinking…");
    setMessages((m) => [...m, { role: "user", text: q }, { role: "assistant", text: "Thinking from your source…" }]);
    try {
      const response = await fetch("/api/chat", { method: "POST", headers: { "content-type": "application/json" }, body: JSON.stringify({ question: q, source, sourceName: name, mode }) });
      const data = await response.json().catch(() => ({}));
      if (!response.ok) throw new Error(data?.error || `AI request failed with ${response.status}.`);
      const answer = typeof data?.answer === "string" ? data.answer : "The AI returned no answer.";
      setMessages((m) => { const next = [...m]; const index = next.length - 1; if (next[index]?.role === "assistant") next[index] = { role: "assistant", text: answer }; else next.push({ role: "assistant", text: answer }); return next; });
      setStatus("AI ready");
    } catch (e) {
      const message = e instanceof Error ? e.message : "Unable to generate an AI response.";
      setMessages((m) => { const next = [...m]; const index = next.length - 1; const fallback = { role: "assistant" as const, text: message }; if (next[index]?.role === "assistant") next[index] = fallback; else next.push(fallback); return next; });
      setError(message); setStatus("AI error");
    } finally { setProcessing(false); }
  };
  ...
  return <main className="min-h-screen bg-background text-foreground">
    ...
    <div className="relative mx-auto min-h-screen max-w-[1600px] p-4 sm:p-5 lg:p-6">
      ...
      <div className="mt-5 grid gap-5 xl:grid-cols-[minmax(300px,0.85fr)_minmax(420px,1fr)_320px]">
        ...
        <section className="flex min-h-0 flex-col rounded-[2rem] border border-hairline bg-surface/65 p-5 sm:p-6"><div className="flex items-center justify-between"><div><p className="text-xs uppercase tracking-[0.18em] text-muted-foreground/80">{mode} workspace</p><h2 className="mt-1 font-display text-lg font-semibold">Ask your source</h2></div><span className="flex items-center gap-1.5 rounded-full border border-primary/15 bg-primary/7 px-2.5 py-1 text-[10px] text-primary-glow"><Bot className="size-3" /> Grounded AI</span></div><div className="mt-4 flex flex-wrap gap-2">{actions[mode].map((a) => <button key={a} disabled={processing} onClick={() => ask(`Run ${a}.`)} className="rounded-xl border border-hairline bg-background/25 px-3 py-2 text-[10px] text-muted-foreground hover:border-primary/20 disabled:opacity-50">{a}</button>)}</div><div className="mt-4 flex-1 space-y-3 overflow-auto rounded-2xl border border-hairline bg-background/20 p-4">{messages.map((m, i) => <div key={i} className={`flex ${m.role === "user" ? "justify-end" : "justify-start"}`}><div className={`max-w-[88%] rounded-2xl px-4 py-3 text-xs leading-6 ${m.role === "user" ? "bg-primary text-primary-foreground" : "border border-hairline bg-surface text-muted-foreground"}`}>{m.text}</div></div>)}</div><div className="mt-4 flex flex-wrap gap-2">{suggestions[mode].map((s) => <button key={s} disabled={processing} onClick={() => ask(s)} className="rounded-xl border border-hairline bg-background/25 px-3 py-2 text-[10px] text-muted-foreground disabled:opacity-50">{s}</button>)}</div><div className="mt-4 flex items-center gap-2 rounded-2xl border border-hairline bg-background/35 p-2"><MessageSquare className="ml-2 size-4 text-muted-foreground" /><input disabled={processing} value={question} onChange={(e) => setQuestion(e.target.value)} onKeyDown={(e) => e.key === "Enter" && ask()} className="min-w-0 flex-1 bg-transparent px-2 py-2 text-xs outline-none" placeholder={processing ? "Thinking…" : "Ask anything about this source…"} /><button disabled={processing} onClick={() => ask()} className="flex size-9 items-center justify-center rounded-xl bg-primary text-primary-foreground disabled:opacity-50"><Send className="size-3.5" /></button></div></section>
